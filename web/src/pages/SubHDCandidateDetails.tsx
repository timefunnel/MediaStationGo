import { CalendarDays, Download, ExternalLink, ThumbsUp, User } from 'lucide-react'

import type { SubtitleSearchCandidate } from '../api/subtitles'

export function SubHDCandidateDetails({ candidate }: { candidate: SubtitleSearchCandidate }) {
  const tags = [candidate.source_type, ...candidate.language_tags, ...candidate.formats].filter(Boolean)
  return (
    <div className="mt-3 border-t border-sand-100 pt-3">
      <div className="flex flex-wrap items-start justify-between gap-2">
        ...
        {candidate.subtitle_group && <span className="rounded bg-blue-50 px-2 py-1 text-xs text-blue-600">{candidate.subtitle_group}</span>}
      </div>
      {tags.length > 0 && (
