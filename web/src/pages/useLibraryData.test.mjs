...
test('筛选传递到服务端；失败显式报错，刷新只发一次新请求并更新统计', async () => {
  ...
  assert.equal(h.requests[1].options.q, '英雄')
  assert.equal(h.requests[1].options.sort, 'rating')
  assert.equal(h.requests[1].options.category, '日韩剧')
  assert.equal(h.requests[1].options.genre, '动作')
  assert.equal(h.requests[1].options.year, '2026')
  assert.equal(h.requests[1].options.language, 'zh')
