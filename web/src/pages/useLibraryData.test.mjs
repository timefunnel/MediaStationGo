...
test('首屏先请求48张卡片，facet 聚合在后台独立完成', async () => {
  const h = harness()
  await h.flush()
  assert.equal(h.requests.length, 1)
  assert.equal(h.requests[0].options.page, 1)
  assert.equal(h.requests[0].options.facets, 0)
  h.requests[0].resolve(pageResponse(1))
  await h.flush()
  assert.equal(h.result.total, 177)
  assert.equal(h.result.seriesCards.length, 48)
  assert.equal(h.result.loadingPage, false)
  assert.equal(h.result.facets, null)
  assert.equal(h.requests.length, 2)
  assert.equal(h.requests[1].options.page, 1)
  assert.equal(h.requests[1].options.facets, 1)
  assert.equal(h.requests[1].options.facet_only, 1)
  h.requests[1].resolve(pageResponse(1, true))
  await h.flush()
  assert.equal(h.result.facets.categories[0].count, 177)
  h.setOptions({ page: 2 })
  await h.flush()
  assert.equal(h.requests.length, 3)
  assert.equal(h.requests[2].options.facets, 0)
  assert.equal(h.result.total, 177)
  assert.equal(h.result.loadingPage, true)
  h.requests[2].resolve(pageResponse(2))
  await h.flush()
...
test('快速翻页取消旧请求，即使旧响应迟到也不能覆盖新页', async () => {
  ...
  h.requests[3].resolve(pageResponse(3)); await h.flush()
  h.requests[2].resolve(pageResponse(2)); await h.flush()
...
test('末页修正和新入库定位同步URL后不重复请求、不出现二次过渡', async () => {
  for (const options of [{ page: 99 }, { page: 1, focus_media: 'new-media' }]) {
    ...
    h.requests[0].resolve(pageResponse(4)); await h.flush(); h.requests[1].resolve(pageResponse(4, true)); await h.flush()
...
test('筛选传递到服务端；失败显式报错，刷新只发一次新请求并更新统计', async () => {
  const h = harness()
  await h.flush()
  h.requests[0].resolve(pageResponse(1)); await h.flush(); h.requests[1].resolve(pageResponse(1, true)); await h.flush()
  h.setOptions({ page: 1, q: '英雄', sort: 'rating', category: '日韩剧', genre: '动作', year: '2026', language: 'zh' }); await h.flush()
  assert.equal(h.requests[2].options.q, '英雄')
  assert.equal(h.requests[2].options.sort, 'rating')
  assert.equal(h.requests[2].options.category, '日韩剧')
  assert.equal(h.requests[2].options.genre, '动作')
  assert.equal(h.requests[2].options.year, '2026')
  assert.equal(h.requests[2].options.language, 'zh')
  h.requests[2].reject(new Error('offline')); await h.flush()
  assert.notEqual(h.result.error, '')
  h.result.reloadCurrentLibrary(); await h.flush()
  assert.equal(h.requests.length, 4)
  assert.equal(h.requests[3].options.facets, 0)
  assert.equal(h.getCount, 2)
  h.requests[3].resolve({ ...pageResponse(1), total: 0, series_cards: [] }); await h.flush()
  assert.equal(h.requests.length, 5)
  assert.equal(h.requests[4].options.facets, 1)
  h.requests[4].resolve(pageResponse(1, true)); await h.flush()
