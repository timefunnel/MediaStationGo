...
func TestPipelineIngestStableTreeAllowsActiveWalkPastVisibilityDeadline(t *testing.T) {
	...
	upstream := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if req.Path == "/115/movie/Pack" {
			...
			case <-time.After(150 * time.Millisecond):
				...
				_ = json.NewEncoder(w).Encode(map[string]any{
					"code": 200, "message": "success",
					"data": map[string]any{"content": []map[string]any{{"name": "Movie.mkv", "size": 5000}}, "total": 1},
				})
			case <-r.Context().Done():
	...
	job = waitPipelineIngestJob(t, svc, job.ID)
	if elapsed := time.Since(startedAt); elapsed < svc.maxConvergence {
		t.Fatalf("active walk completed before the visibility deadline: %s", elapsed)
	}
	if job.Status != PipelineIngestStatusCompleted || job.Result.Convergence == nil || job.Result.Convergence.Status != PipelineIngestStatusCompleted {
		t.Fatalf("job=%#v", job)
	}
	if activeWalkCanceled.Load() {
		t.Fatal("active OpenList walk was canceled at the visibility deadline")
	}
}

func TestPipelineIngestStableTreeCancelsActiveWalkAtScanTimeout(t *testing.T) {
	var activeWalkCanceled atomic.Bool
	upstream := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if r.URL.Path != "/api/fs/list" {
			t.Fatalf("unexpected openlist api request %s", r.URL.Path)
	...
	svc.stableWindow = 10 * time.Millisecond
	svc.maxConvergence = time.Minute
	svc.maxScanDuration = 100 * time.Millisecond
	...
	if elapsed := time.Since(startedAt); elapsed > 2*time.Second {
		t.Fatalf("active walk exceeded configured scan timeout: %s", elapsed)
	}
	if job.Status != PipelineIngestStatusNeedsAttention || job.Result.Convergence == nil || job.Result.Convergence.Status != PipelineIngestStatusNeedsAttention {
		t.Fatalf("job=%#v", job)
	}
	if !strings.Contains(job.Message, "configured scan timeout") {
		t.Fatalf("job message=%q", job.Message)
	}
	if !activeWalkCanceled.Load() {
		t.Fatal("active OpenList walk was not canceled at the configured scan timeout")
	}
}
