...
func TestDanmakuPayloadWaitsForQueuedPlaybackMatch(t *testing.T) {
	...
	prober := &playbackQueueProber{
		...
		result:  &ProbeResult{DurationSec: 1440, Width: 3840, Height: 2160, VideoCodec: "h264", AudioCodec: "aac"},
	}
	svc, resolver := newPlaybackProbeTestService(t, "cloud-danmaku-wait", prober)
	if err := svc.repo.DB.AutoMigrate(&model.MediaDanmaku{}); err != nil {
		t.Fatal(err)
	}
	if err := svc.repo.DB.Model(&model.Media{}).Where("id = ?", "cloud-danmaku-wait").Updates(map[string]any{
		"original_name": "Kaijuu.S01E01.mp4",
		"size_bytes":    int64(1346864288),
		"duration_sec":  1440,
	}).Error; err != nil {
	...
	pipeline := &fakeDanmakuPipeline{
		...
		playbackResult: DanmakuPlaybackMatchResult{
			...
			CommentCache: DanmakuCommentCacheResult{Status: "ready", Count: 1},
		},
		payload: DanmakuPayload{
			Source: "dandanplay", EpisodeID: "175500001", Count: 1, Total: 1,
	...
	request := httptest.NewRequest(http.MethodGet, "http://nas.local/api/stream/cloud-danmaku-wait", nil)
	request.Header.Set("User-Agent", "SenPlayer/1.0")
	response := httptest.NewRecorder()
	if err := svc.ServeFile(response, request, "cloud-danmaku-wait"); err != nil {
		t.Fatal(err)
	}
	if response.Code != http.StatusFound || response.Header().Get("Location") != resolver.link.URL {
		t.Fatalf("playback redirect = status %d location %q", response.Code, response.Header().Get("Location"))
	...
	go func() {
		payload, err := danmaku.Payload(t.Context(), "cloud-danmaku-wait", DanmakuOptions{})
		result <- payloadResult{payload: payload, err: err}
	}()
	select {
	case early := <-result:
		t.Fatalf("danmaku request returned before background match completed: %+v", early)
	case <-time.After(50 * time.Millisecond):
	}
	close(release)
	select {
	case completed := <-result:
		...
		if completed.payload.EpisodeID != "175500001" || len(completed.payload.Comments) != 1 {
			t.Fatalf("unexpected first-play payload: %+v", completed.payload)
		}
	case <-time.After(2 * time.Second):
		t.Fatal("danmaku request did not resume after background match completed")
	}
	select {
	case <-probeStarted:
		// ffprobe 仍被 probeRelease 阻塞；弹幕请求能先完成，证明两者没有错误串联。
	case <-time.After(2 * time.Second):
		t.Fatal("metadata probe did not continue after danmaku match")
	}
}
