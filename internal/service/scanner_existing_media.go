...
func (s *ScannerService) existingCloudMediaSnapshotForLibraries(ctx context.Context, libraryIDs []string) (map[string]existingCloudMedia, error) {
	...
	if err := s.repo.DB.WithContext(ctx).
		Model(&model.Media{}).
		Select("library_id", "path", "title", "original_name", "episode_title", "size_bytes", "duration_sec", "width", "height", "video_codec", "audio_codec", "container", "bit_rate", "video_bit_rate", "frame_rate", "video_profile", "video_range", "video_bit_depth", "audio_bit_rate", "audio_channels", "audio_channel_layout", "audio_sample_rate", "media_probe_version", "poster_url", "backdrop_url", "strm_url", "overview", "year", "release_date", "rating", "tm_db_id", "bangumi_id", "douban_id", "thetvdb_id", "season_num", "episode_num", "genres", "actors", "countries", "languages", "nsfw", "scrape_status").
	...
	for _, row := range rows {
		...
		snapshot[row.Path] = existingCloudMedia{
			...
			MediaProbeVersion:  row.MediaProbeVersion,
