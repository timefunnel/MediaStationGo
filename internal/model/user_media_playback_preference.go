...
// UserMediaPlaybackPreference stores one user's playback selections for one
// playable item or one stable virtual preference scope. No row means the user
// has not configured the scope yet; SubtitleEnabled=false therefore remains an
...
type UserMediaPlaybackPreference struct {
	...
	PreferredMediaSourceID string `gorm:"size:128;not null;default:''" json:"preferred_media_source_id,omitempty"`
