...
func TestEmbyApplyFilmlyResumeCompatibilityLeavesNonEpisodeUnchanged(t *testing.T) {
	gin.SetMode(gin.TestMode)
	w := httptest.NewRecorder()
	c, _ := gin.CreateTestContext(w)
	c.Request = httptest.NewRequest("GET", "/emby/Users/user-1/Items/Resume", nil)
	...
	movie := map[string]any{"Id": "movie-1", "Type": "Movie", "Name": "电影", "Etag": "movie-etag"}
	out := map[string]any{"Items": []map[string]any{movie}}
	if err := embyApplyFilmlyResumeCompatibility(c, out); err != nil {
		t.Fatalf("apply Filmly movie compatibility: %v", err)
	}

	if !reflect.DeepEqual(movie, map[string]any{"Id": "movie-1", "Type": "Movie", "Name": "电影", "Etag": "movie-etag"}) {
