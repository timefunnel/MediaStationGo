...
func registerAuthedMediaRoutes(authed *gin.RouterGroup, svc *service.Container) {
	...
	authed.POST("/media/work-search", requirePermission(svc, "can_play_media"), searchMediaWorksHandler(svc))
	...
	authed.POST("/media/scrape/preview", middleware.AdminRequired(), manualScrapePreviewHandler(svc))
	...
	authed.GET("/media/:id/migration", middleware.AdminRequired(), getMediaMigrationHandler(svc))
	authed.POST("/media/:id/migration/validate", middleware.AdminRequired(), validateMediaMigrationHandler(svc))
	authed.POST("/media/:id/migration/apply", middleware.AdminRequired(), applyMediaMigrationHandler(svc))
